...
public class MapActivity extends BaseActivity implements GoogleMap.OnMarkerClickListener, OnMapReadyCallback {
    ...
    volatile boolean isSharingLoc = false, isBusVolunteer = false, isUserOnline = true, isLocSharingCommandAutoGenerated = false, isDataReady = false;
    ...
    private void initNetworkCallbacks() {
        ...
        volunteerNetworkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(@NonNull Network network) {
                super.onAvailable(network);
                if (userIsOnline() || SharedPref.getString(getApplicationContext(), "routeNo") == null)
                    return;
                setUserOnlineStatus(true);
                ...
                buslocref.addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (total[0] < 4 && total[1] < 100) {
                            total[0] = dataSnapshot.getChildrenCount();
                            total[1]++;
                            if (total[0] < 4 && total[1] < 4) return;
                        }
                        if (Objects.equals(SharedPref.getString(getApplicationContext(), "email"), dataSnapshot.child("currentSharerID").getValue(String.class))) {
                            isLocSharingCommandAutoGenerated = true;
                            if (!isMyServiceRunning(TransmitLocationService.class))
                                runOnUiThread(MapActivity.this::checkForLocationPermissions);
                        } else if (dataSnapshot.child("currentSharerID").getValue(String.class) == null)
                            runOnUiThread(MapActivity.this::checkForLocationPermissions);
                        else buslocref.removeEventListener(this);
                    }
                    ...
                    new CountDownTimer(10000, 1000) {
                        ...
                        public void onTick(long millisUntilFinished) {
                            if (CommonUtils.alerter(MapActivity.this)) {
                                showNotification(2, "Your internet connection was interrupted.", "Please reconnect in " + millisUntilFinished / 1000 + (millisUntilFinished == 1000 ? " second." : " seconds."), MapActivity.this, new Intent(MapActivity.this, MapActivity.class));
                            } else {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (isSharingLoc) {
                        if (s != null && !s.equals("null") && !s.equals(userId)) {
                            ...
                            if (!isLocSharingCommandAutoGenerated)
                    ...
                    isLocSharingCommandAutoGenerated = false;
                }
